...
func TestFilterTargetNodes(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "when any key-value filter matches, node should be targeted",
			nodeLabels:   map[string]string{"k1": "v1"},
			annotations:  map[string]string{ServiceAnnotationLoadBalancerTargetNodeLabels: "k1=v1"},
			nodeTargeted: true,
		},
		{
			name:         "when any just-key filter matches, node should be targeted",
			nodeLabels:   map[string]string{"k1": "v1", "k2": "v2"},
			annotations:  map[string]string{ServiceAnnotationLoadBalancerTargetNodeLabels: "k2"},
			nodeTargeted: true,
		},
		{
